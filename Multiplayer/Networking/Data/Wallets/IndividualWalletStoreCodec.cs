...
public static class IndividualWalletStoreCodec
{
    ...
    public static JObject Write(IndividualWalletLedger ledger)
    {
        ...
        var initialized = new JArray();
        foreach (var id in ledger.SnapshotInitialized()) initialized.Add(id.ToString("N"));
        return new JObject { ["Version"] = 2, ["Balances"] = balances, ["Initialized"] = initialized, ["Operations"] = operations };
    }
    public static bool TryRead(JObject value, IndividualWalletLedger ledger)
    {
        ...
        int version = (int?)value["Version"] ?? 0;
        if ((version != 1 && version != 2) || value["Balances"] is not JObject balances || value["Operations"] is not JArray operations || version == 2 && value["Initialized"] is not JArray) return false;
        var parsedBalances = new List<KeyValuePair<Guid, double>>(); var parsedOperations = new List<WalletOperationRecord>();
        var parsedInitialized = new List<Guid>();
        ...
        if (version == 2)
            foreach (var token in (JArray)value["Initialized"]) { if (token.Type != JTokenType.String || !Guid.TryParseExact((string)token, "N", out var id)) return false; parsedInitialized.Add(id); }
        return ledger.TryReplace(parsedBalances, parsedOperations, version == 1 ? null : parsedInitialized);
    }
}
